...
local function position_for(stop)
  local right = RIGHT[stop.direction] or RIGHT[defines.direction.north]
  local p = stop.position
  -- 1,5 Felder: die Haltestelle ist 2 × 2 groß, so steht die Ausgabe direkt an ihrer Kante.
  return { x = p.x + right[1] * 1.5, y = p.y + right[2] * 1.5 }
end
...
function Output.ensure(station)
  ...
  local position = position_for(stop)
  ...
  local found = surface.find_entities_filtered({ area = area, name = C.station_output })[1]
  if not found then
    for _, ghost in pairs(surface.find_entities_filtered({ area = area, ghost_name = C.station_output })) do
      ...
      found = revived or found
    end
  end
  local output = found or surface.create_entity({
